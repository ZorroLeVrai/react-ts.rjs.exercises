...
describe(Task, () => {
  it.each([
    ...
    ["2h", "3600s", 50]
      ...
      render(<Task totalTime={totalTime} timeToComplete={timeToComplete} taskStatus={TaskStatus.IN_PROGRESS} taskName="My Task" isFirst={false} isLast={false} />);
  ...
  it("should not display the move to top icon", () => {
    render(<Task totalTime="2d" timeToComplete="1d" taskStatus={TaskStatus.IN_PROGRESS} taskName="My Task" isFirst={true} isLast={false} />);
    expect(screen.queryByTestId("task_movetop")).not.toBeInTheDocument();
  });

  it("should not display the move to bottom icon", () => {
    render(<Task totalTime="2d" timeToComplete="1d" taskStatus={TaskStatus.IN_PROGRESS} taskName="My Task" isFirst={false} isLast={true} />);
  ...
  it("should display the move to bottom and the move to top icons", () => {
    render(<Task totalTime="2d" timeToComplete="1d" taskStatus={TaskStatus.IN_PROGRESS} taskName="My Task" isFirst={false} isLast={false} />);
